"""MAX-side stats aggregation for the daily digest.

:class:`MaxStatsService` walks the per-user match set, buckets matches
by status and returns a :class:`UserStats` snapshot. The clock is
injected so the ``applied_today`` boundary is deterministic in tests
and overridable in production.

The MAX-side service mirrors :class:`apply_pilot.features.telegram.digest.service.StatsService`
but delegates the channel account enumeration to
:class:`apply_pilot.features.max.repository.MaxAccountRepository` (via
:func:`get_all_users_with_max`) instead of the Telegram account repo.
The per-user aggregation logic is identical: it consumes
:class:`VacancyMatch` rows by user id, groups by
:class:`MatchStatus`, and counts matches with ``updated_at`` on the
digest date (UTC) toward ``applied_today``.
"""
...
class MaxStatsService:
    ...
    @property
    def max_account_repo(self) -> _MaxAccountRepo:
        return self._max_account_repo

    @property
    def user_repo(self) -> _UserRepo:
    ...
    def get_user_stats(
        ...
        *,
        ...
        The method is intentionally sync: there is no ``await`` in the
        body. Mirrors the Telegram-side ``StatsService.get_user_stats``
        contract so the channel-agnostic renderer is callable with
    ...
    async def get_all_users_with_max(self) -> list[User]:
        """Return every :class:`User` that has a linked MAX account.

        The implementation lists the (small) set of MAX accounts and
        resolves each one through the user repository. Users with no
        row in :class:`UsersRepository` are skipped — they would
        otherwise fail the foreign-key check on the SQL side and are
        not broadcast targets.

        Mirrors
        :meth:`~apply_pilot.features.telegram.digest.service.StatsService.get_all_users_with_telegram`
        but iterates the MAX account rows instead.
        """
        accounts = self._max_account_repo.list_all()
        users: list[User] = []
        for account in accounts:
            user = self._user_repo.get_by_id(account.user_id)  # type: ignore[attr-defined]
            if user is not None:
                users.append(user)
        return users
