...
:class:`DigestRunner` extends :class:`~apply_pilot.runtime.process.BaseProcess`
...
_LOGGER = logging.getLogger("apply_pilot.features.telegram.digest.runner")
...
class DigestRunner(BaseProcess):
    """A :class:`BaseProcess` that fires the digest once a day at *digest_hour_utc*.

    Defaults: ``name="digest-runner"``, ``digest_hour_utc=9``. The
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(name="digest-runner", signals=(signal.SIGINT, signal.SIGTERM))
        self._sender = sender
    ...
    async def run_once(self, *, on_date: date | None = None) -> int:
        ...
        Exposed for tests and the manual ``POST /digest/send`` endpoint
        so callers can trigger an iteration without spinning up the
        scheduler.
        """
        ...
        try:
            ...
            _LOGGER.exception(
                "digest.run_once.failed",
                extra={"event": "digest.run_once.failed", "on_date": target_date.isoformat()},
            )
            return 0
        _LOGGER.info(
            "digest.run_once.sent",
            extra={
                "event": "digest.run_once.sent",
