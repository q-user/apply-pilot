...
every :class:`~job_apply.features.sources.adapter.SourceAdapter` call:
...
sits in the same
:class:`~job_apply.features.sources.adapter.AdapterRegistry` slot as
the inner adapter.
...
:class:`~job_apply.features.audit.service.AuditService` collaborator:
...
if TYPE_CHECKING:
    # ``ApplyResult`` lives in :mod:`job_apply.features.apply_worker.runtime`,
    # which transitively imports :mod:`job_apply.features.sources` (via the
...
_LOG_PREFIX = "job_apply.features.sources.breaker."
...
class SourceCircuitRegistry(Protocol):
    ...
    :class:`BreakeredSourceAdapter` wiring in :mod:`job_apply.app`,
...
class BreakeredSourceAdapter:
    ...
    :class:`~job_apply.features.sources.adapter.AdapterRegistry` in
    ...
    * State transitions emit audit events on the optional
    ...
    :class:`~job_apply.features.apply_worker.retry.ApplyRetryPolicy`'s
