...
* :meth:`SourceAdapter.extract_screening_questions` — build
  :class:`~job_apply.features.screening.models.ScreeningQuestion` rows
  from a raw dict.
...
:mod:`job_apply.features.hh.adapter` (``HhSourceAdapter``); this module
...
if TYPE_CHECKING:
    ...
    # which transitively imports :mod:`job_apply.features.sources.models`. The
...
class SourceQuery:
    ...
    (e.g. :class:`~job_apply.features.hh.search.HHQuery` for the hh
...
class AdapterRegistry:
    ...
    (e.g. from :mod:`job_apply.app`) and looked up by name at request
    time.
