...
:class:`~job_apply.features.apply_worker.models.ApplyJob`, pick the
right adapter, dispatch, and walk the lifecycle (``succeeded`` /
requeued / ``dead_letter``). The :class:`ApplyWorkerProcess` wraps
that loop in a :class:`~job_apply.runtime.process.BaseProcess` so the
...
_LOG_PREFIX = "job_apply.features.apply_worker.runtime."
...
class ApplyResult:
    ...
    error:
        ...
        the :class:`~job_apply.features.apply_worker.models.ApplyJob`
