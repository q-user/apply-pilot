...
* a :class:`~job_apply.features.telegram.repository.TelegramAccountRepository`
  — to resolve the local ``user_id`` to a Telegram chat id;
* a :class:`~job_apply.features.telegram.bot.TelegramBot` — to actually
...
class TelegramApplyNotifier:
    ...
    :class:`~job_apply.features.telegram.repository.InMemoryTelegramAccountRepository`
    and a recording stand-in for
    :class:`~job_apply.features.telegram.bot.TelegramBot`; production
    wiring in :mod:`job_apply.features.apply_worker.api` (or the
...
def _build_message(*, status: str, job: ApplyJob) -> str:
    ...
    :class:`~job_apply.features.apply_worker.models.ApplyStatusHistory`
