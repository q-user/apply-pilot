...
* :class:`~job_apply.features.sources.normalizer.VacancyNormalizer` —
  ...
  :class:`~job_apply.features.sources.models.Vacancy` (delegates to
...
* An :class:`~job_apply.features.apply_worker.runtime.ApplyAdapter`
  with ``name == "hh"`` — the apply submission transport (the
  production
  :class:`~job_apply.features.hh.apply.HhApplyAdapter` or a fake in
  tests).
...
:class:`~job_apply.features.sources.adapter.AdapterRegistry` instead
...
class HhSourceAdapter:
    ...
    hh-flavored
    :class:`~job_apply.features.hh.search.HHQuery` is the adapter's
    ...
    *not* forwarded — the typed ``HHQuery`` is the contract, and
    ...
    #: :attr:`~job_apply.features.sources.models.Vacancy.source` for
    #: hh-ingested rows and the key the
    #: :class:`~job_apply.features.sources.adapter.AdapterRegistry`
    ...
    async def search(self, query: SourceQuery) -> list[dict[str, Any]]:
        ...
        :class:`~job_apply.features.hh.search.HHQuery` is the only
        ...
        from job_apply.features.hh.search import HHQuery
    ...
    async def apply(self, job: ApplyJob) -> ApplyResult:
        ...
        :class:`~job_apply.features.hh.apply.HhApplyAdapter` (the
