"""Pre-warm the import chain for the ``messaging`` test slice.
...
(``pytest tests/features/messaging/``).

This conftest pre-loads the modules in a benign order so the cycle
resolves before pytest tries to import the test modules. The
pre-warm is a no-op when the modules are already loaded, so the
full suite pays no extra cost.
