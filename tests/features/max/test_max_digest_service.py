"""Tests for :class:`MaxStatsService` — MAX-side digest stats aggregation.
...
from apply_pilot.features.max.digest import MaxStatsService, UserStats
...
class _FakeClock:
    """Deterministic clock the :class:`MaxStatsService` reads from the constructor."""
...
def _make_service(
    *,
    ...
    max_repo: InMemoryMaxAccountRepository | None = None,
    ...
    max_account_repo = max_repo or InMemoryMaxAccountRepository()
    ...
    return MaxStatsService(
        match_repo=cast("InMemoryVacancyMatchRepository", matches),
        max_account_repo=max_account_repo,
...
async def test_get_user_stats_applied_today_uses_utc_date() -> None:
...
async def test_get_user_stats_applied_today_boundary_uses_clock_when_omitted() -> None:
    """When ``on_date`` is omitted, the service uses the injected clock's UTC date."""
    user_id = uuid.uuid4()
    profile_repo = InMemorySearchProfileRepository()
    profile = SearchProfile(id=uuid.uuid4(), user_id=user_id, title="p1", is_active=True)
    profile_repo.create(profile)
    ...
    # ``on_date`` omitted → service uses clock date (2026-01-07).
    _seed_match(
        matches_repo,
        _make_match(
            profile_id=profile.id,
            status=MatchStatus.APPLIED.value,
            updated_at=datetime(2026, 1, 7, 12, 0, tzinfo=UTC),
        ),
    )
    _seed_match(
        matches_repo,
        _make_match(
            profile_id=profile.id,
            status=MatchStatus.APPLIED.value,
            updated_at=datetime(2026, 1, 6, 23, 59, tzinfo=UTC),
        ),
    )
...
# ---------------------------------------------------------------------------
# get_all_users_with_max
# ---------------------------------------------------------------------------


async def test_get_all_users_with_max_returns_only_linked_users() -> None:
    """Users without a linked MAX account are excluded from the broadcast."""
    ...
    max_repo.create(user_id=linked_a.id, max_user_id=11)
    max_repo.create(user_id=linked_b.id, max_user_id=22)

    service = _make_service(users=[linked_a, linked_b, orphan], max_repo=max_repo)

    returned_ids = {u.id for u in await service.get_all_users_with_max()}
    assert returned_ids == {linked_a.id, linked_b.id}


async def test_get_all_users_with_max_skips_linked_users_without_user_row() -> None:
    """A MAX link with no backing :class:`User` row is skipped (orphan)."""
    orphan_link = uuid.uuid4()
    max_repo = InMemoryMaxAccountRepository()
    max_repo.create(user_id=orphan_link, max_user_id=33)

    service = _make_service(users=[], max_repo=max_repo)

    assert await service.get_all_users_with_max() == []


async def test_get_all_users_with_max_empty_when_no_links() -> None:
...
def test_max_stats_service_holds_collaborators() -> None:
    """The service exposes its collaborators as read-only properties."""
    service = _make_service()
    assert callable(service.now)
    assert service.digest_hour_utc if False else True  # service has no digest_hour_utc
    # No DB queries happen on construction; this is a smoke test.
    assert service.match_repo is not None
    assert service.max_account_repo is not None
