"""Tests for :class:`MaxDigestRunner` — scheduled daily tick loop.
...
from apply_pilot.features.max.digest import MaxDigestRunner
...
def test_max_digest_runner_is_base_process_subclass() -> None:
    """``MaxDigestRunner`` must extend ``BaseProcess`` to wire into the runtime."""
    assert issubclass(MaxDigestRunner, BaseProcess)


@pytest.mark.asyncio
async def test_max_digest_runner_run_once_invokes_sender_with_today() -> None:
    """``run_once`` calls the sender with the clock's date."""
    runner = MaxDigestRunner(
...
@pytest.mark.asyncio
async def test_max_digest_runner_run_once_returns_sender_count() -> None:
...
@pytest.mark.asyncio
async def test_max_digest_runner_run_once_logs_and_returns_zero_on_exception(
    caplog: pytest.LogCaptureFixture,
) -> None:
    """A sender exception is logged and ``run_once`` returns ``0``."""
    ...
    with caplog.at_level("ERROR", logger="apply_pilot.features.max.digest.runner"):
        sent = await runner.run_once()
...
def test_max_digest_runner_deadline_future_sleeps() -> None:
    """When the clock is before ``digest_hour_utc``, the deadline is tomorrow."""
    runner = MaxDigestRunner(
        sender=_FakeSender(),  # type: ignore[arg-type]
        now=_ControllableClock(datetime(2026, 6, 15, 7, 0, tzinfo=UTC)),
        sleep=_RecordingSleep(),  # type: ignore[arg-type]
        digest_hour_utc=9,
    )

    wait_seconds = runner._seconds_until_next_deadline()  # noqa: SLF001

    # 2 hours until 09:00 UTC the same day.
    assert wait_seconds == pytest.approx(2 * 3600.0)


def test_max_digest_runner_deadline_past_returns_non_positive() -> None:
    """When the clock is past ``digest_hour_utc``, the deadline is tomorrow."""
    runner = MaxDigestRunner(
        sender=_FakeSender(),  # type: ignore[arg-type]
    ...
    # 23 hours until 09:00 UTC the next day.
    assert wait_seconds == pytest.approx(23 * 3600.0)
...
@pytest.mark.asyncio
async def test_max_digest_runner_loop_dispatches_once_when_deadline_is_past() -> None:
...
@pytest.mark.asyncio
async def test_max_digest_runner_does_not_dispatch_before_digest_hour() -> None:
...
def test_max_digest_runner_signals_include_sigterm() -> None:
...
def test_max_digest_runner_default_digest_hour_is_nine() -> None:
...
def test_max_digest_runner_default_name_is_max_digest_runner() -> None:
    """The default ``name`` distinguishes the MAX runner from the Telegram one."""
    runner = MaxDigestRunner(
        sender=_FakeSender(),  # type: ignore[arg-type]
        now=_ControllableClock(datetime(2026, 6, 15, 8, 0, tzinfo=UTC)),
        sleep=_RecordingSleep(),  # type: ignore[arg-type]
    )
    assert runner.name == "max-digest-runner"


def test_max_digest_runner_holds_collaborators() -> None:
